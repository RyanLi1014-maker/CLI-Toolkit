        ...
        if self.confg_file_path.exists():  # Check if the configuration file exists
            with open(self.confg_file_path, "r") as f:
                try:
                    ...
                    self.logger.warning(
                        f"File '{self.confg_file_path.name}' is not a valid json file. Returning default configuration."
                    )
            ...
            self.logger.warning(
                f"File '{self.confg_file_path.name}' does not exist. Returning default configuration and saving it to the file."
            )
            self.update(
                self.default
